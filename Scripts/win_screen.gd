...
func _ready():
	# Ensure the tree isn't paused and mouse is visible
	...
	# Create a beautiful entrance animation sequence
	...
	tween.tween_property(buttons_container, "modulate:a", 1.0, 1.0).set_delay(1.5)
	
	# Add a continuous subtle pulse to the title
...
func _pulse_title_forever():
	...
	pulse_tween.tween_property(title, "scale", Vector2(1.05, 1.05), 2.0)
	pulse_tween.tween_property(title, "scale", Vector2(1.0, 1.0), 2.0)
